...
class NIDaqBackend(AcquisitionBackend):
    ...
    def _start_multi_device(self, on_chunk: ChunkCallback) -> None:
        ...
        try:
            ...
            self._apply_reference_clock(tasks, prefer_100mhz)

            if any(device_info.get(dev) and device_info[dev].is_dsa for dev in tasks):
                self._apply_sync_pulse(tasks, master_dev)
    ...
    def _apply_reference_clock(self, tasks: dict[str, "nidaqmx.Task"], prefer_100mhz: bool) -> None:
        """Locks every device's timebase to the chassis backplane clock so they don't
        free-run on independent oscillators and drift apart over a long acquisition.
        Tried on every task including the master; failures are non-fatal since some
        buses/devices won't support a given source -- falls back to the next candidate,
        and to no shared clock at all (sync then relies on the start trigger alone)."""
        candidates = (
        ...
        self.sync_report.append(
            "Reference clock: none applied -- devices run on independent timebases and may drift."
        )
        logger.warning(
            "Could not apply a shared reference clock across %s -- devices may drift apart "
            "over a long acquisition.", list(tasks)
        )

    def _apply_sync_pulse(self, tasks: dict[str, "nidaqmx.Task"], master_dev: str) -> None:
        ...
        if not applied:
            return
        self.sync_report.append(f"Sync pulse /{master_dev}/SyncPulse -> {applied} slave task(s)")
